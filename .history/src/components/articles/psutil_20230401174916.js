...
import solid from './articles.css'
...
<pre id="93b9" class="graf graf--pre graf-after--p"><code className='language-python'>
    In [1]: psutil.cpu_stats() {"\n"}
    Out[1]: scpustats(ctx_switches=437905181, interrupts=2222556355L, soft_interrupts=0, syscalls=109468308) {"\n"}
</code></pre>
<p id="28a8" class="graf graf--p graf-after--pre">We can get some info about the disk and the memory state:</p>
<pre id="97c8" class="graf graf--pre graf-after--p"><code className='language-python'>
In [1]: psutil.disk_usage("c:") {"\n"}
Out[1]: sdiskusage(total=127950385152L, used=116934914048L, free=11015471104L, percent=91.4) {"\n"}
                   </code></pre>
<pre id="de21" class="graf graf--pre graf-after--pre"><code className='language-python'>
In [2]: psutil.virtual_memory() {"\n"}
Out[2]: svmem(total=8488030208L, {"\n"}
              available=3647520768L, {"\n"}
              percent=57.0, {"\n"}
              used=4840509440L, {"\n"}
              free=3647520768L)</code></pre>
...
<pre id="1854" class="graf graf--pre graf-after--p"><code className='language-python'>
In [1]: psutil.sensors_battery() {"\n"}
Out[1]: sbattery(percent=77, secsleft=18305, power_plugged=False)</code></pre>
<pre id="2385" class="graf graf--pre graf-after--pre"> <code className='language-python'>
In [2]: psutil.sensors_temperatures() # In Celsius {"\n"}
Out[2]: {"{'ACPI\\ThermalZone\\THM0_0':"} {"\n"}
 {" [shwtemp(label='',  current=49.05000000000001, high=127.05000000000001, critical=127.05000000000001)]}"}
</code>
</pre>
<hr class={solid}/>
...
<pre id="eec0" class="graf graf--pre graf-after--p"><code className='language-python'>In [1]: p = psutil.Process(9800)</code></pre>
<p id="3604" class="graf graf--p graf-after--pre">Then, we can access all the information and statistics of the process:</p>
<pre id="eeb9" class="graf graf--pre graf-after--p"> <code className='language-python'>
In [1]: p.exe()
Out[1]: 'C:\\Windows\\System32\\dllhost.exe'</code></pre>
<pre id="397c" class="graf graf--pre graf-after--pre"><code className='language-python'>
In [2]: p.cpu_percent()
Out[2]: 0.0</code></pre>
<pre id="108f" class="graf graf--pre graf-after--pre"><code className='language-python'>
In [3]: p.cwd()
Out[3]: 'C:\\WINDOWS\\system32'</code></pre>
...
<pre id="de9e" class="graf graf--pre graf-after--p"><code className='language-python'>
        ...
        The *kind* parameter filters for connections that fit the following criteria:</code></pre>
<pre id="3d78" class="graf graf--pre graf-after--pre"> <code className='language-python'>
+------------+----------------------------------------------------+{"\n"}
...
<pre id="4588" class="graf graf--pre graf-after--p"><code className='language-python'>
    In [1]: def link_connection_to_process(): {"\n"}
    ...:     for connection in ps.net_connections(): {"\n"}
    ...:         try: {"\n"}
    ...:             yield [ps.Process(pid=connection.pid).name(), connection] {"\n"}
    ...:         except ps.AccessDenied: {"\n"}
    ...:             continue # Keep going if we don't have access</code></pre>
...
<pre id="15e5" class="graf graf--pre graf-after--p"><code className='language-python'>
    In [1]: for proc_to_con in ps.net_connections(): {"\n"}
    ...:     print proc_to_con{"\n"}
    ...:     raw_input("..."){"\n"}
    ...:{"\n"}
['ManagementServer.exe', sconn(fd=-1, family=2, type=1, laddr=addr(ip='127.0.0.1', port=5905)
, raddr=addr(ip='127.0.0.1', port=49728), status='ESTABLISHED', pid=5224)]
...</code></pre>
